...
void my_fun_CC1101_test1(void)
{
		...
		else if(my_CC1101_all_step == 0X0052)
    {
        ...
        printf("========CC1101 ALarm is finish!!!!==yaoxin==\n\n");
...
//==========周期、报警处理部分===========
    if(temp_status == 0x01 || temp_status == 0x02) //遥信--01为周期，02为报警
    {
        ...
        if(temp_status == 0x02)
        {
            ...
            //报警数据发送状态记录
            if(my_indicator_alarm_data[my_indicator_index].duanlu_data != 0 && my_CC1101_COM_Fram_buf[9]==0) //历史是报警，本次为撤销报警
                my_indicator_alarm_data[my_indicator_index].TX_status_duanlu = 0x01; //  //表示有报警数据产生，还没有发送。如果发送完了，把这个字节清零
								...
								my_indicator_alarm_data[my_indicator_index].TX_status_duanlu = 0x01;  //有报警产生
            else
                my_indicator_alarm_data[my_indicator_index].TX_status_duanlu = 0x02;   //没有历史报警，也没有新报警
    ...
    else if(temp_status == 0x53 ) //AC_record
    {
        ...
        for(ii = 0; ii < my_length; ii++)
            my_indicator_record_data[my_indicator_index].my_wave_record_I_buf[ii + 11] = my_CC1101_COM_Fram_buf[9 + ii];  //录波数据
...
uint8_t my_fun_GPRS_RX_test1(void) //此函数为结束函数，收到OK帧后，结束对话过程
{
    ...
    {
			#if User_CC1101_record_status==0
        //报警发送成功，清除报警发送状态
			...
			else if(my_indicator_alarm_data[my_indicator_tx_index].TX_status_duanlu==0x02 ||my_indicator_alarm_data[my_indicator_tx_index].TX_status_jiedi==0x02)
			{
				my_indicator_alarm_data[my_indicator_tx_index].TX_status_duanlu=0X00;
				my_indicator_alarm_data[my_indicator_tx_index].TX_status_jiedi=0X00;
			}
